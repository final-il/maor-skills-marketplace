...
**Comment formatting — links must use bare URLs on their own line (or surrounded by whitespace), not `[label](url)` markdown nor `[label|url]` wiki syntax.** Jira's UI renders both bracket forms as literal text, but auto-linkifies bare `https://...` URLs. Recommended pattern: one logical line per link, label on the same line followed by the URL — e.g. `Discovery doc: https://github.com/.../discovery.md`. Issue keys (`CSI-396`) are also auto-linkified.
