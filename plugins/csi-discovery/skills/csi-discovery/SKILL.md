...
**Comment formatting — write URLs as plain bare URLs with NO bracket wrapping at all.** Both `[label](url)` markdown and `[label|url]` wiki-link syntax render as literal text in this Jira instance (the MCP→ADF conversion strips the link mark). The reliable pattern is just: `Label: https://...` on its own line, separated from other content by blank lines. Jira auto-linkifies bare URLs and issue keys (`CSI-396`).
